...
import { FileText, Plus, MapPin, Sparkles, ArrowRight, CreditCard, Headphones, Ban, Search } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Flatten and shuffle programs for random display
        const flattened = unis.flatMap((uni: any) =>
    ...
    <div className="space-y-8">
                ...
                <div key={app.id} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      `}>
                        {app.universityName?.name ? app.universityName.name.substring(0, 1) : 'U'}
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{app.universityName?.name}</h3>
                        <p className="text-slate-500 dark:text-slate-400">{app.programName?.name}</p>
                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 dark:text-slate-500">
                          <span className="font-mono">APP-{app.id.substring(app.id.length - 6)}</span>
                          <span>•</span>
      ...
      <div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Available Universities</h2>
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-500 w-full md:w-80 transition-all shadow-sm"
            />
          </div>
        </div>
        ...
        {allPrograms.filter(item =>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allPrograms.filter(item =>
              item.university.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
              item.program.name.toLowerCase().includes(searchQuery.toLowerCase())
            ).map(({ university, program }) => (
              <div key={`${university.id}-${program.id}`} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm hover:shadow-md transition-all group flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg font-bold ${university.logoColor}`}>
                      {university.name.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 line-clamp-1">{university.name}</h4>
                  ...
                  <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider rounded border border-slate-200 dark:border-slate-700">
                    #{university.ranking}
                  </span>
                </div>

                <div className="mb-4 flex-1">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors mb-2">
                    {program.name}
                  </h3>

                  <div className="grid grid-cols-2 gap-y-2 text-sm">
                    <div className="text-slate-500 dark:text-slate-400">
                      <span className="block text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Degree</span>
                      {program.degree || 'Bachelor'}
                    </div>
                    <div className="text-slate-500 dark:text-slate-400">
                      <span className="block text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Duration</span>
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div>
                    <span className="block text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Tuition Fee</span>
                    <span className="text-lg font-bold text-slate-900 dark:text-slate-100">₹{Number(program.fee).toLocaleString()}</span>
                  </div>
                  <Link to="/student/apply">
                    <Button size="sm">
                      Apply Now
          ...
          <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">No programs found</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-1">
